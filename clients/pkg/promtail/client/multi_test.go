...
import (
	...
	metrics2 "github.com/grafana/loki/clients/pkg/promtail/client/metrics"
...
var (
	nilMetrics = metrics2.NewMetrics(nil, nil)
	metrics    = metrics2.NewMetrics(prometheus.DefaultRegisterer, nil)
)
...
func TestMultiClient_Handle_Race(t *testing.T) {
	...
	//m.start()
	//
	//m.Chan() <- api.Entry{Labels: model.LabelSet{"foo": "bar", metrics2.ReservedLabelTenantID: "1"}, Entry: logproto.Entry{Line: "foo"}}
	//
	//m.Stop()
}
